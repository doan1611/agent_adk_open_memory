...
class GaussModel(BaseLlm):
    ...
    def _parse_tool_calls_to_adk_format(self, response_text: str) -> types.Content:
        ...
        try:
            ...
            # Nếu có content field chứa JSON string function call
            if isinstance(response_data, dict) and "content" in response_data:
                content_str = response_data.get("content", "")
                if content_str:
                    try:
                        # Parse content string như JSON
                        content_data = json.loads(content_str)
                        
                        # Nếu là function call format với function và arguments
                        if isinstance(content_data, dict) and "function" in content_data and "arguments" in content_data:
                        ...
                        # Nếu là transfer_to_agent format
                        elif isinstance(content_data, dict) and "agent_name" in content_data:
                            agent_name = content_data.get("agent_name", "")
                            request_data = content_data.get("request", {})
                            
                            if agent_name == "ingest_agent" and request_data:
